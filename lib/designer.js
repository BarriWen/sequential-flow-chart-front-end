(function (global, factory) {
	...
	function appendPlaceholder(g, x, y) {
	    ...
	    const startX = x + PH_WIDTH / 2 - SIZE$2 / 8;
	    const startY = y + PH_HEIGHT / 2 - SIZE$2 / 8;
	    const endX = x + PH_WIDTH / 2 + SIZE$2 / 8;
	    const endY = y + PH_HEIGHT / 2 + SIZE$2 / 8;
	...
	class ContainerStepComponentView {
	    ...
	    static create(parent, step, configuration) {
	        ...
	        Dom.translate(sequenceComponent.view.g, PADDING_X$2, PADDING_TOP$1 + LABEL_HEIGHT$2);
	        const width = sequenceComponent.view.width + PADDING_X$2 * 2;
	        const height = sequenceComponent.view.height + PADDING_TOP$1 + LABEL_HEIGHT$2;
	...
	class SwitchStepComponentView {
	    ...
	    static create(parent, step, configuration) {
	        ...
	        const boxHeight = ICON_SIZE$2 + PADDING_Y$1 * 2;
	        ...
	        branchNames.forEach((branchName, i) => {
	            ...
	            LabelView.create(g, offsetX + joinXs[i] + PADDING_X$1, PADDING_TOP + LABEL_HEIGHT + CONNECTION_HEIGHT + boxHeight / 2, branchName, 'secondary');
	            ...
	            JoinView.createStraightJoin(g, new Vector(containerOffsets[i] + joinXs[i] + PADDING_X$1, PADDING_TOP + LABEL_HEIGHT * 2 + CONNECTION_HEIGHT + boxHeight / 2), 120);
	            Dom.translate(sequence.view.g, sequenceX, sequenceY);
	        });
	        // LabelView.create(g, containerWidths[0], PADDING_TOP, step.name);
	        const g1 = Dom.svg("g");
	        const text = Dom.svg('text', {
	            x: ICON_SIZE$2 + containerWidths[0] - PADDING_X$1 * 2,
	            y: boxHeight / 2 + PADDING_TOP,
	            class: 'sqd-task-text'
	        });
	        text.textContent = "If/Else";
	        g1.appendChild(text);
	        const textWidth = Math.max(text.getBBox().width + PADDING_X$1 * 2 + ICON_SIZE$2, MIN_TEXT_WIDTH$1);
	        const boxWidth = ICON_SIZE$2 + PADDING_X$1 * 3 + textWidth;
	        const rect = Dom.svg('rect', {
	        ...
	        Dom.attrs(icon, {
	            x: containerWidths[0] - textWidth + PADDING_X$1,
	            y: PADDING_TOP * 1.5,
	            width: ICON_SIZE$2,
	            height: ICON_SIZE$2
	        });
	        ...
	        Dom.attrs(moreIcon, {
	            class: 'more',
	            id: Date.now(),
	            x: containerWidths[0] + 2 * PADDING_X$1,
	            y: PADDING_TOP * 1.5,
	            width: ICON_SIZE$2,
	            height: ICON_SIZE$2
	        });
	        ...
	        Dom.attrs(icon1, {
	            class: "moreicon",
	            id: `icon1${Date.now()}`,
	            x: containerWidths[0] + 2 * PADDING_X$1 + ICON_SIZE$2 + 30,
	            y: PADDING_TOP * 1.5,
	            width: ICON_SIZE$2,
	            height: ICON_SIZE$2
	        });
	        ...
	        Dom.attrs(icon2, {
	            class: "moreicon",
	            id: `icon2${Date.now()}`,
	            x: containerWidths[0] + 2 * PADDING_X$1 + ICON_SIZE$2 + 10,
	        ...
	        JoinView.createJoins(g, new Vector(containerWidths[0], PADDING_TOP + LABEL_HEIGHT + boxHeight / 2), containerOffsets.map((o, i) => new Vector(o + joinXs[i] + PADDING_X$1, PADDING_TOP + LABEL_HEIGHT + CONNECTION_HEIGHT + boxHeight / 2)));
	...
	class TaskStepComponentView {
	    ...
	    static create(parent, step, configuration) {
	        ...
	        const boxWidth = ICON_SIZE$1 + PADDING_X * 3 + textWidth;
	        const rect = Dom.svg('rect', {
	            x: 0.5,
	            y: 0.5,
	            class: 'sqd-task-rect',
