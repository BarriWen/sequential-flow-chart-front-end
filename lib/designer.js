(function (global, factory) {
	...
	class SwitchStepComponentView {
	    ...
	    static create(parent, step, configuration) {
	        ...
	        const DROPDOWN_Y = 80;
	        const DROPDOWN_X1 = containerWidths[0] - 180; // 
	        const DROPDOWN_X2 = containerWidths[0] - 54; // 
	        const DROPDOWN_X3 = containerWidths[0] + 63; // 
	        const g1 = Dom.svg("g");
	        const text = Dom.svg("text", {
	            x: ICON_SIZE$6 + containerWidths[0] - PADDING_X$5 / 2 - 187,
	        ...
	        const textRight = Dom.svg("text", {
	            x: ICON_SIZE$6 + containerWidths[0] - 37,
	            y: boxHeight / 2.0 + PADDING_TOP,
	            class: "sqd-task-text",
	        });
	        ...
	        Dom.attrs(moreIcon, {
	            class: "moreIcon",
	            x: ICON_SIZE$6 + containerWidths[0] + PADDING_X$5 + textWidth + 43,
	            y: PADDING_TOP * 1.2,
	            width: ICON_SIZE$6,
	            height: ICON_SIZE$6,
	        });
	        ...
	        const rightCopyImgContainerCircle = Dom.svg("rect", {
	            ...
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 90,
	            y: PADDING_Y$5 + 10, // 
	        });
	        ...
	        Dom.attrs(changeIcon, {
	            ...
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 93,
	            y: PADDING_Y$5 + 14,
	            width: ICON_SIZE$6,
	            height: ICON_SIZE$6,
	        });
	        ...
	        const rightDeleteImgContainerCircle = Dom.svg("rect", {
	            class: "sqd-task-ImgContainerCircle",
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 70,
	            y: PADDING_Y$5 + 40,
	        });
	        ...
	        Dom.attrs(deleteIcon, {
	            ...
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 73,
	            y: PADDING_Y$5 + 43,
	            width: 22,
	            height: 22,
	        });
	        ...
	        Dom.attrs(reminder1, {
	            ...
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 107,
	            y: PADDING_Y$5 - 18, // -25 -> -8
	        });
	        const reminderText1 = Dom.svg("text", {
	            class: "sqd-task-text",
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 107 + 13,
	            y: PADDING_Y$5 - 6,
	        });
	        ...
	        Dom.attrs(reminder2, {
	            ...
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 127,
	            y: PADDING_Y$5 + 13,
	        });
	        const reminderText2 = Dom.svg("text", {
	            class: "sqd-task-text",
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 127 + 10,
	            y: PADDING_Y$5 + 13 + 12,
	        });
	        ...
	        const reminderText3 = Dom.svg("text", {
	            class: "sqd-task-text",
	            x: containerWidths[0] + 5 * PADDING_X$5 + 3 * ICON_SIZE$6 + 107 + 6,
	            y: PADDING_Y$5 + 43 + 12,
	        });
	        ...
	        const dropdownBoxBottomShapeRecMain1 = Dom.svg("rect", {
	            width: DROPDOWN1_W,
	...
	function tagDropDown(dropdown, h, w, textToChange) {
	    ...
	    // Field names
	    const rect1 = createRect$1("sqd-task-rect", 0.5, h, w, 3 * h, `dropdown${Date.now()}`, RECT_RADIUS$2);
	    const nameText = addTxt$1("Select tag: ", PADDING_X$2 * 2, 1.5 * h + 10);
	    ...
	    const dropdownBoxShape = createRect$1("option select-field", startX + wid + PADDING_X$2 + 30, // 88.75
	    startY, 110, 25, "", 4);
	    Dom.attrs(dropdownBoxShape, {
	        fill: "#fff",
	        stroke: "#a0a0a0",
	    });
	    const dropdownBoxShapeAfter = createRect$1("option select-field", startX + wid + PADDING_X$2 + 30, startY, 110, 25, `dropdownBoxShape${Date.now()}`);
	    Dom.attrs(dropdownBoxShapeAfter, {
	        fill: "#fff",
	        stroke: "#a0a0a0",
	        opacity: 0,
	    ...
	    // Default value
	    const dropdownBoxInnerText = addTxt$1("Any Tag", startX + wid + 4 * PADDING_X$2 + PADDING_X$2 / 2 - 2, // 94.75
	    startY + 12); // x = 45.25
	    gSubDropdownbox.appendChild(dropdownBoxInnerText);
	    ...
	    new Request(`http://localhost:8080/tag/${userID}`, { method: 'GET' });
	    let tags = [];
	    tags = ["Any tag", "Tag 1", "Tag 2", "Tag 3"];
	    const editTags = function (tags) {
	        const dropdownBoxBottomShape = createRect$1("option select-field", startX - PADDING_X$2 / 2 - 4, startY + 30, 110, 25 * tags.length + 10, "", 4);
	        ...
	        for (let i = 0; i < tags.length; i++) {
	            const dropdownBoxBottomShapeText = addTxt$1(tags[i], startX, startY + 25 * (i + 1) + 21);
	            // y = 81.75
	            const dropdownBoxBottomShapecover = createRect$1("option select-field choice", startX - PADDING_X$2 / 2 + 1, startY + 25 * (i + 1) + 10, 100, 25, `dropdownBoxBottomShapecover${Date.now()}`, 4);
	            Dom.attrs(dropdownBoxBottomShapecover, {
	                fill: "#fff",
	                opacity: 0.3,
	            });
	...
	class EmailComponentView {
	    ...
	    static create(parent, step, configuration) {
	        ...
	        addDropDown(gDropdown, 1.5 * boxHeight, boxWidth, upCheckIcon, newSend, newSub);
	        g.appendChild(gRightPop3Reminder);
	        g.appendChild(gUpPop3);
	        g.appendChild(setUpReminder);
	        // Add EventListeners
	...
	function addDropDown(dropdown, h, w, button, send, sub, cont) {
	    ...
	    dropdown.appendChild(gSubDropdownbox);
	    const rect1 = createRect("sqd-task-rect sqd-hidden", // Temporary hidden 
	    0.5, h - 15, w, 4 * h + 5 * PADDING_Y, `dropdown${Date.now()}`, RECT_RADIUS);
	    gSubDropdownbox.appendChild(rect1);
	    const rect2 = createRect("sqd-task-rect", 0.5, h - 15, w, 4 * h + PADDING_Y, `dropdown${Date.now()}`, RECT_RADIUS);
	    ...
	    const classicIcon = Dom.svg("image", {
	            href: classIconUrl,
	        })
	        ;
