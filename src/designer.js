(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      (global.sequentialWorkflowDesigner = factory()));
})(this, function () {
  "use strict";
  ...
  class DesignerContext {
    ...
    tryDeleteStep(step, choice) {
      ...
      this.notifiyDefinitionChanged(true);
      if (
        ((_a = this.selectedStep) === null || _a === void 0
          ? void 0
          : _a.id) === step.id
      ) {
  ...
  function promptChoices(context, action, event) {
    ...
    btn1.addEventListener("click", function (e) {
      ...
      // Copy behavior
        ...
        // Copy true branch
        if (
          context.selectedStep.branches.True.length > 0 &&
          (output == 0 || output == 2)
        ) {
          for (let i = 0; i < context.selectedStep.branches.True.length; i++) {
            ...
            step.id =
              "copy-" +
              context.selectedStep.branches.True[i].id +
              "-at-" +
              Date.now();
        ...
        // Copy false branch
        if (
          context.selectedStep.branches.False.length > 0 &&
          (output == 1 || output == 2)
        ) {
          for (let i = 0; i < context.selectedStep.branches.False.length; i++) {
            ...
            step.id =
              "copy-" +
              context.selectedStep.branches.False[i].id +
              "-at-" +
              Date.now();
        ...
        duplicateStep.id =
          "copy-" + context.selectedStep.id + "-at-" + Date.now();
        context.behaviorController.start(
          pos,
          DragStepBehavior.create(context, duplicateStep)
        );
      }
      const designer = document.getElementById("designer");
      designer.removeChild(designer.childNodes[1]);
  ...
  class ControlBar {
    ...
    static create(parent, context) {
      ...
      context.onSelectedStepChanged.subscribe(() =>
        bar.onSelectedStepChanged()
      );
      context.onIsMoveModeEnabledChanged.subscribe((i) =>
        bar.onIsMoveModeEnabledChanged(i)
      );
      return bar;
    }
  ...
  class GlobalEditor {
    ...
    static create(definition, context) {
      ...
      const content = context.configuration.editors.globalEditorProvider(
        definition,
        editorContext
      );
  ...
  class StepEditor {
    ...
    static create(step, context) {
      ...
      const content = context.configuration.editors.stepEditorProvider(
        step,
        editorContext
      );
  ...
  class SmartEditor {
    ...
    static create(parent, context) {
      ...
      context.onSelectedStepChanged.subscribe((s) =>
        editor.onSelectedStepChanged(s)
      );
      context.onDefinitionChanged.subscribe(() => editor.onDefinitionChanged());
      context.onIsSmartEditorCollapsedChanged.subscribe((ic) =>
        view.setIsCollapsed(ic)
      );
      return editor;
    }
    ...
    tryRender(step) {
      if (this.currentStep !== step) {
        const editor = step
          ? StepEditor.create(step, this.context)
          : GlobalEditor.create(this.context.definition, this.context);
    ...
    static createJoins(parent, start, targets) {
      for (const target of targets) {
        ...
        const join = Dom.svg("path", {
          class: "sqd-join",
          fill: "none",
          d: `M ${start.x} ${start.y} q ${x * c * 0.3} ${y * c * 0.8} ${
            x * c
          } ${y * c} l ${x * l} 0 q ${x * c * 0.7} ${y * c * 0.2} ${x * c} ${
            y * c
          }`,
        });
        parent.insertBefore(join, parent.firstChild);
  ...
  class SequenceComponentView {
    ...
    static create(parent, sequence, configuration) {
      ...
      components.forEach((e) => {
        ...
        if (e.step.name == "Time Delay") {
          if (e.step.properties["Time"]) {
            e.view.g.childNodes[3].innerHTML =
              "Delay for " +
              e.step.properties["Time"] +
              " " +
              e.step.properties["Delay"];
            e.view.g.childNodes[6].childNodes[5].innerHTML =
              e.step.properties["Delay"];
      ...
      if (components[i - 1] instanceof TaskStepComponent) {
        ...
        Dom.translate(
          stop,
          maxJoinX - PH_WIDTH / 6.8,
          offsetY - PH_HEIGHT / 16
        );
  ...
  class SequenceComponent {
    ...
    static create(parent, sequence, configuration) {
      const view = SequenceComponentView.create(
        parent,
        sequence,
        configuration
      );
  ...
  class LabelView {
    static create(parent, x, y, text, theme) {
      ...
      const nameWidth = Math.max(
        nameText.getBBox().width + LABEL_PADDING_X * 2,
        MIN_LABEL_WIDTH
      );
...
*/
  ...
  class SwitchStepComponentView {
    ...
    static create(parent, step, configuration) {
      ...
      const containerWidths = sequenceComponents.map(
        (s) => Math.max(s.view.width, MIN_CHILDREN_WIDTH) + PADDING_X$1 * 2
      );
      ...
      branchNames.forEach((branchName, i) => {
        ...
        JoinView.createStraightJoin(
          g,
          new Vector(
            containerOffsets[i] + joinXs[i] + PADDING_X$1,
            PADDING_TOP + LABEL_HEIGHT * 2 + CONNECTION_HEIGHT + boxHeight / 2
          ),
          PH_HEIGHT
        );
      ...
      //const iconUrl = configuration.iconUrlProvider ? configuration.iconUrlProvider(step.componentType, step.type) : null;
      const inputView = InputView.createRoundInput(
        g,
        containerWidths[0],
        0,
        iconUrl
  ...
  class SwitchStepComponent {
    ...
    getPlaceholders(result) {
      if (this.currentState !== StepComponentState.dragging) {
        this.view.sequenceComponents.forEach((sc) =>
          sc.getPlaceholders(result)
        );
      }
    }
    setIsDragging(isDragging) {
      ...
      // isDragging = false;
      if (this.currentState !== StepComponentState.dragging) {
        this.view.sequenceComponents.forEach((s) =>
    ...
    static create(parent, step, parentSequence, configuration) {
      switch (step.componentType) {
        case ComponentType.task:
          return TaskStepComponent.create(
            parent,
            step,
            parentSequence,
            configuration
        ...
        case ComponentType.switch:
          return SwitchStepComponent.create(
            parent,
            step,
            parentSequence,
            configuration
        ...
        case ComponentType.container:
          return ContainerStepComponent.create(
            parent,
            step,
            parentSequence,
            configuration
  ...
  class DragStepView {
    ...
    static create(step, configuration) {
      ...
      const fakeSequence = [];
      const stepComponent = StepComponentFactory.create(
        canvas,
        step,
        fakeSequence,
        configuration.steps
    ...
    onStart(position) {
      ...
      if (this.movingStepComponent) {
        ...
        const clientPosition =
          this.movingStepComponent.view.getClientPosition();
      ...
      this.state = {
        ...
        finder: PlaceholderFinder.create(
          this.context.getPlaceholders(),
          this.context
        ),
        offset,
      };
    ...
    onEnd(interrupt) {
      ...
      if (!interrupt && this.currentPlaceholder) {
        if (this.movingStepComponent) {
          ...
          modified = this.context.tryInsertStep(
            this.step,
            this.currentPlaceholder.parentSequence,
            this.currentPlaceholder.index
          );
  ...
  class ToolboxItem {
    ...
    static create(parent, step, context) {
      ...
      const view = ToolboxItemView.create(
        parent,
        step,
        context.configuration.steps
      );
    ...
    startDrag(position) {
      if (!this.context.isReadonly) {
        ...
        this.context.behaviorController.start(
          position,
          DragStepBehavior.create(this.context, newStep)
        );
      }
    }
  }
  ...
  class ToolboxView {
    ...
    setIsCollapsed(isCollapsed) {
      ...
      this.headerToggleIcon.innerHTML = isCollapsed
        ? Icons.arrowDown
        : Icons.close;
      if (!isCollapsed) {
  ...
  class Toolbox {
    ...
    static create(parent, context) {
      ...
      toolbox.render();
      context.onIsToolboxCollapsedChanged.subscribe((ic) =>
        toolbox.onIsToolboxCollapsedChanged(ic)
      );
  ...
  class StartComponentView {
    ...
    destroy() {
      var _a;
      (_a = this.g.parentNode) === null || _a === void 0
        ? void 0
        : _a.removeChild(this.g);
    }
  }
...
*/
  ...
  class WorkspaceView {
    ...
    bindMouseDown(handler) {
      this.canvas.addEventListener(
        "mousedown",
        (e) => handler(readMousePosition(e), e.target, e.button),
        false
      );
    }
    bindTouchStart(handler) {
  ...
  class Workspace {
    ...
    render() {
      ...
      this.view.render(
        ...
        this.context.definition.properties.journeyId
      );
    ...
    zoom(direction) {
      ...
      const scale = this.context.limitScale(
        this.context.viewPort.scale + delta
      );
        ...
        if (clickedStep.step.componentType === "switch") {
          ...
          if (clickedStep.view.g.childNodes[13].childNodes[3].id) {
            const switchMoreButtonId =
              clickedStep.view.g.childNodes[13].childNodes[3].id.toString();
            const switchMoreButton =
              document.getElementById(switchMoreButtonId);
          ...
          if (clickedStep.view.g.childNodes[14].childNodes[1].id) {
            const deleteButtonId =
              clickedStep.view.g.childNodes[14].childNodes[1].id.toString();
          ...
          if (clickedStep.view.g.childNodes[14].childNodes[2].id) {
            const dropdownButId =
              clickedStep.view.g.childNodes[14].childNodes[2].id.toString();
          ...
          //subdropdown
          if (
            clickedStep.view.g.childNodes[17].childNodes[0].childNodes[3].id
          ) {
            const dropdownButId =
              clickedStep.view.g.childNodes[17].childNodes[0].childNodes[3].id.toString();
            const dropdownBut = document.getElementById(dropdownButId);
            dropdownBut.onclick = function (e) {
              console.log(3498, clickedStep.view.g.childNodes[17].childNodes);
              e.stopPropagation();
              clickedStep.view.g.childNodes[17].childNodes[1].classList.toggle(
                "sqd-hidden"
              );
            };
          }
          //subdropdown1
          if (
            clickedStep.view.g.childNodes[16].childNodes[0].childNodes[3].id
          ) {
            const dropdownButId =
              clickedStep.view.g.childNodes[16].childNodes[0].childNodes[3].id.toString();
            const dropdownBut = document.getElementById(dropdownButId);
            dropdownBut.onclick = function (e) {
              console.log(3498, clickedStep.view.g.childNodes[16].childNodes);
              e.stopPropagation();
              clickedStep.view.g.childNodes[16].childNodes[1].classList.toggle(
                "sqd-hidden"
              );
            };
          }
          ...
          const selectRunUpperId =
            clickedStep.view.g.childNodes[17].childNodes[1].childNodes[2].id.toString();
          const selectRunUpper = document.getElementById(selectRunUpperId);
          selectRunUpper.onclick = function () {
            //console.log(3589, clickedStep)
            const showVal =
              clickedStep.view.g.childNodes[17].childNodes[1].childNodes[1]
                .innerHTML;
          ...
          const selectRunUpperId1 =
            clickedStep.view.g.childNodes[17].childNodes[1].childNodes[5].id.toString();
          const selectRunUpper1 = document.getElementById(selectRunUpperId1);
          selectRunUpper1.onclick = function () {
            const showVal =
              clickedStep.view.g.childNodes[17].childNodes[1].childNodes[4]
                .innerHTML;
          ...
          const selectRunLowerId =
            clickedStep.view.g.childNodes[16].childNodes[1].childNodes[2].id.toString();
          ...
          selectRunLower.onclick = function () {
            ...
            const showVal =
              clickedStep.view.g.childNodes[16].childNodes[1].childNodes[1]
                .innerHTML;
            clickedStep.view.g.childNodes[16].childNodes[0].childNodes[2].textContent =
              showVal;
            clickedStep.view.g.childNodes[16].childNodes[1].classList.toggle(
              "sqd-hidden"
            );
          ...
          const selectRunLowerId1 =
            clickedStep.view.g.childNodes[16].childNodes[1].childNodes[5].id.toString();
          const selectRunLower1 = document.getElementById(selectRunLowerId1);
          selectRunLower1.onclick = function () {
            //console.log(3589, )
            const showVal =
              clickedStep.view.g.childNodes[16].childNodes[1].childNodes[4]
                .innerHTML;
            console.log(
              3596,
              clickedStep.view.g.childNodes[16].childNodes[0].childNodes[2]
            );
            clickedStep.view.g.childNodes[16].childNodes[0].childNodes[2].textContent =
              showVal;
            clickedStep.view.g.childNodes[16].childNodes[1].classList.toggle(
          ...
          if (clickedStep.view.g.childNodes[14].childNodes[0]) {
            console.log(
              "duplicate if",
              clickedStep.view.g.childNodes[14].childNodes[0].id
            );
            const duplicateId =
              clickedStep.view.g.childNodes[14].childNodes[0].id.toString();
          ...
          button.onclick = function () {
            ...
            if (show !== undefined) {
              ...
              if (show && inputval !== "") {
                ...
                clickedStep.step.properties["Time"] =
        ...
        if (clickedStep.step.componentType === "task") {
          ...
          if (clickedStep.step.name == "Time Delay") {
            const timedelaydropdownId =
              clickedStep.view.g.childNodes[6].childNodes[7].id;
            const timedelaydropdownClassList =
              clickedStep.view.g.childNodes[6].childNodes[8];
            const timedelaySubdropdownDayId =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[2].id;
            const timedelaySubdropdownHourId =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[5].id;
            const timedelaySubdropdownMinuteId =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[8].id;
            const timedelaydropdownShow =
              clickedStep.view.g.childNodes[6].childNodes[5];
            const timedelayDayString =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[1]
                .innerHTML;
            const timedelayHourString =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[4]
                .innerHTML;
            const timedelayMinuteString =
              clickedStep.view.g.childNodes[6].childNodes[8].childNodes[7]
                .innerHTML;
            ...
            const showDropdownButtonId =
              clickedStep.view.g.childNodes[5].childNodes[2].childNodes[1].id;
            ...
            const copyButtonId =
              clickedStep.view.g.childNodes[5].childNodes[0].childNodes[1].id;
            ...
            handleTimedelayDropdownButtonClick(
              timedelaydropdownId,
              timedelaydropdownClassList
            );
            //duplicate
            const uppopupCheckButId =
              clickedStep.view.g.childNodes[8].childNodes[0].childNodes[1].id;
            //const uppopupCheckBut = document.getElementById(uppopupCheckButId);
            handleTimedelayDropdownButtonClick(
              uppopupCheckButId,
              clickedStep.view.g.childNodes[6],
              undefined,
              undefined,
              undefined,
              undefined,
            ...
            const uppopupduplicateButId =
              clickedStep.view.g.childNodes[8].childNodes[2].childNodes[1].id;
            ...
            //delete
            const deleteButtonId =
              clickedStep.view.g.childNodes[5].childNodes[1].childNodes[1].id.toString();
            const deleteButton = document.getElementById(deleteButtonId);
            const uppopupdeleteButtonId =
              clickedStep.view.g.childNodes[8].childNodes[1].childNodes[1].id;
            ...
            handleMouseover(
              deleteButton,
              "mouseover",
              clickedStep.view.g.childNodes[7].childNodes[2]
            );
            handleMouseover(
              deleteButton,
              "mouseout",
            ...
            handleMouseover(
              duplicateBut,
              "mouseover",
              clickedStep.view.g.childNodes[7].childNodes[1]
            );
            handleMouseover(
              duplicateBut,
              "mouseout",
            ...
            handleMouseover(
              showDropdownButton,
              "mouseover",
              clickedStep.view.g.childNodes[7].childNodes[0]
            );
            handleMouseover(
              showDropdownButton,
              "mouseout",
            ...
            if (clickedStep.view.g.childNodes[5].childNodes[1]) {
              const deleteButtonId =
                clickedStep.view.g.childNodes[5].childNodes[1].id.toString();
              const deleteButton = document.getElementById(deleteButtonId);
              //add mouseover event
              deleteButton.addEventListener("mouseover", function () {
                //console.log(3752, clickedStep.view.g.childNodes)
                clickedStep.view.g.childNodes[7].childNodes[2].classList.remove(
                  "sqd-hidden"
                );
              });
              deleteButton.addEventListener("mouseout", function () {
                //console.log(3650, 'mouseout')
                clickedStep.view.g.childNodes[7].childNodes[2].classList.add(
                  "sqd-hidden"
                );
              });
              deleteButton.onclick = function () {
            ...
            if (clickedStep.view.g.childNodes[5].childNodes[2].id) {
              const dropdownButId =
                clickedStep.view.g.childNodes[5].childNodes[2].id.toString();
              //add mouseover event
              const dropdownBut = document.getElementById(dropdownButId);
              dropdownBut.addEventListener("mouseover", function () {
                clickedStep.view.g.childNodes[7].childNodes[0].classList.remove(
                  "sqd-hidden"
                );
              });
              dropdownBut.addEventListener("mouseout", function () {
                clickedStep.view.g.childNodes[7].childNodes[0].classList.add(
                  "sqd-hidden"
                );
              });
              dropdownBut.onclick = function (e) {
                ...
                clickedStep.view.g.childNodes[6].classList.toggle("sqd-hidden");
                clickedStep.view.g.childNodes[8].classList.toggle("sqd-hidden");
                clickedStep.view.g.childNodes[10].classList.toggle(
                  "sqd-hidden"
                );
                clickedStep.view.g.childNodes[11].classList.toggle(
                  "sqd-hidden"
                );
              };
            }
            ...
            if (clickedStep.view.g.childNodes[10].childNodes[1]) {
              const subDropdownButtonId =
                clickedStep.view.g.childNodes[10].childNodes[0].childNodes[3].id.toString();
              ...
              subDropdownButton.onclick = function () {
                ...
                clickedStep.view.g.childNodes[10].childNodes[1].classList.toggle(
                  "sqd-hidden"
                );
              };
            }
            ...
            if (clickedStep.view.g.childNodes[11].childNodes[1]) {
              const subDropdownButtonId1 =
                clickedStep.view.g.childNodes[11].childNodes[0].childNodes[3].id.toString();
              const subDropdownButton1 =
                document.getElementById(subDropdownButtonId1);
              subDropdownButton1.onclick = function () {
                clickedStep.view.g.childNodes[11].childNodes[1].classList.toggle(
                  "sqd-hidden"
                );
              };
            }
            //upper subdropdown
            const selectRunUpperId =
              clickedStep.view.g.childNodes[11].childNodes[1].childNodes[2].id.toString();
            const selectRunUpper = document.getElementById(selectRunUpperId);
            //console.log(4125, clickedStep.view.g.childNodes[8].childNodes[1].childNodes[2])
            selectRunUpper.onclick = function () {
              //console.log(4091, fakeThis)
              const showVal =
                clickedStep.view.g.childNodes[11].childNodes[1].childNodes[1]
                  .innerHTML;
              ...
              clickedStep.view.g.childNodes[11].childNodes[0].childNodes[2].textContent =
                showVal;
              clickedStep.view.g.childNodes[3].textContent = showVal;
              clickedStep.view.g.childNodes[8].childNodes[1].classList.toggle(
                "sqd-hidden"
              );
              clickedStep.step.properties["Select List"] = showVal;
            ...
            const selectRunUpperId1 =
              clickedStep.view.g.childNodes[11].childNodes[1].childNodes[5].id.toString();
            const selectRunUpper1 = document.getElementById(selectRunUpperId1);
            selectRunUpper1.onclick = function () {
              const showVal =
                clickedStep.view.g.childNodes[11].childNodes[1].childNodes[4]
                  .innerHTML;
            ...
            //lower subdropdown
            const selectRunLowerId =
              clickedStep.view.g.childNodes[10].childNodes[1].childNodes[2].id.toString();
            const selectRunLower = document.getElementById(selectRunLowerId);
            selectRunLower.onclick = function () {
              //console.log(4117, clickedStep.view.g.childNodes)
              const showVal =
                clickedStep.view.g.childNodes[10].childNodes[1].childNodes[1]
                  .innerHTML;
              clickedStep.view.g.childNodes[10].childNodes[0].childNodes[2].textContent =
                showVal;
              clickedStep.view.g.childNodes[10].childNodes[1].classList.toggle(
                "sqd-hidden"
              );
            ...
            const selectRunLowerId1 =
              clickedStep.view.g.childNodes[10].childNodes[1].childNodes[5].id.toString();
            const selectRunLower1 = document.getElementById(selectRunLowerId1);
            selectRunLower1.onclick = function () {
              //console.log(3589, )
              const showVal =
                clickedStep.view.g.childNodes[10].childNodes[1].childNodes[4]
                  .innerHTML;
              console.log(
                3596,
                clickedStep.view.g.childNodes[10].childNodes[0].childNodes[2]
              );
              clickedStep.view.g.childNodes[10].childNodes[0].childNodes[2].textContent =
                showVal;
              clickedStep.view.g.childNodes[10].childNodes[1].classList.toggle(
            ...
            if (clickedStep.view.g.childNodes[5].childNodes[0]) {
              const duplicateId =
                clickedStep.view.g.childNodes[5].childNodes[0].id.toString();
              const duplicateBut = document.getElementById(duplicateId);
              duplicateBut.addEventListener("mouseover", function () {
                console.log(3752, clickedStep.view.g.childNodes);
                clickedStep.view.g.childNodes[7].childNodes[1].classList.remove(
                  "sqd-hidden"
                );
              });
              duplicateBut.addEventListener("mouseout", function () {
                //console.log(3650, 'mouseout')
                clickedStep.view.g.childNodes[7].childNodes[1].classList.add(
                  "sqd-hidden"
                );
              });
            ...
            if (clickedStep.view.g.childNodes[8].childNodes[2]) {
              const duplicateId =
                clickedStep.view.g.childNodes[8].childNodes[2].id.toString();
              const duplicateBut = document.getElementById(duplicateId);
              const tempContext = this.context;
              duplicateBut.onclick = function (e) {
                // e.preventDefault();
    ...
    onWheel(e) {
      ...
      const position = mouseRealPoint
        .multiplyByScalar(-newScale)
        .add(mousePoint);
    ...
    trySelectStep(step) {
      if (this.selectedStepComponent) {
        ...
        if (this.selectedStepComponent.step.componentType === "task") {
          ...
          this.selectedStepComponent.view.g.childNodes[5].classList.add(
            "sqd-hidden"
          );
          ...
          this.selectedStepComponent.view.g.childNodes[14].classList.add(
            "sqd-hidden"
          );
          this.selectedStepComponent.view.g.childNodes[15].classList.add(
            "sqd-hidden"
          );
          this.selectedStepComponent.view.g.childNodes[16].classList.add(
            "sqd-hidden"
          );
          this.selectedStepComponent.view.g.childNodes[17].classList.add(
            "sqd-hidden"
          );
        }
        this.selectedStepComponent = null;
  ...
  class DesignerView {
    ...
    destroy() {
      ...
      (_b = this.root.parentElement) === null || _b === void 0
        ? void 0
        : _b.removeChild(this.root);
    }
    onResize() {
      this.reloadLayout();
    }
        ...
        case ComponentType.container:
          {
            ...
            if (
              containerStep.sequence === needle ||
              find(containerStep.sequence, needle, result)
            ) {
              result.push(step);
              return true;
            }
  ...
  class Designer {
    ...
    getSelectedStepId() {
      var _a;
      return (
        ((_a = this.context.selectedStep) === null || _a === void 0
          ? void 0
          : _a.id) || null
      );
    }
