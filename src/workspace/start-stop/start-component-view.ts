...
export class StartComponentView implements ComponentView {
	public static create(parent: SVGElement, sequence: Sequence, configuration: StepsConfiguration): StartComponentView {
		const g = Dom.svg('g');
...
function createCircle(parent: SVGElement, x: number, y: number, text: string): SVGGElement {
	let g = Dom.svg("g", {
		class: "sqd-start",
		id: 'start'
	});
