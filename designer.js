        ...
        if(clickedStep.step.componentType === 'switch'){
					...
					if(clickedStep.view.g.childNodes[15].childNodes[1].childNodes[2].id){
						...
						dropdownBut.onclick = function(e){
							...
							// 克隆最后一个SubDropdownGroupInfo,最后一个gGroupLogicDropdown
							...
							clickedStep.view.g.childNodes[15].appendChild(duplicateNode)
							clickedStep.view.g.childNodes[15].appendChild(duplicateNode1)

							// 取最后一个condition（一般为倒数第二位node），改变其位置（原始位置与复制前重合）
							const newGroupInfo = clickedStep.view.g.childNodes[15].childNodes[clickedStep.view.g.childNodes[15].childNodes.length-2].childNodes
							//console.log(7440, newGroupInfo)
							for(let i = 0; i < newGroupInfo.length; i++){
								// 改变AddConditionButton, ConditionLogicDropdown位置
								if(i == 0){
									const newGroupButton = newGroupInfo[i].childNodes
									for(let j = 0; j < newGroupButton.length; j++){
										// const cordy = parseInt(newGroupButton[j].getAttribute('y'))
										const originY = parseInt(clickedStep.view.g.childNodes[15].childNodes[clickedStep.view.g.childNodes[15].childNodes.length-3].childNodes[0].getAttribute('y'))
										newGroupButton[j].setAttribute('y', originY + 15)
									}
								}
							}
